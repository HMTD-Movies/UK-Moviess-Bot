...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            ...
            if not files:
                await client.send_message(req_channel,f"<b>#Requested_Movie 👇🏻\n\n➟ 🎥 Movie Name :- {search}\n➟ 💁🏻 Requested By :- {message.from_user.first_name}\n➟ 🆔 User ID :- {message.from_user.id}\n\n➟ 👨🏻‍✈️ User Link :- {message.from_user.mention}</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✔️ Upload Done", callback_data="close_data")]]))
